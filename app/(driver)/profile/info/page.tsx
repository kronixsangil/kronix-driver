  ...
  return (
    ...
    >
      {finalSrc ? (
        ...
        <div className="grid h-full w-full place-items-center text-sm font-extrabold">{initials}</div>
      )}
    </div>
  );
}
...
export default function DriverInfoPage() {
  ...
  const avatarSrc = useMemo(
    () => buildOfficialDriverPhotoSrc(form.profileImageUrl, form.fullName),
    [form.profileImageUrl, form.fullName]
  );
    ...
    <div className="w-full bg-slate-50 p-0">
      <div className="mx-auto w-full max-w-md px-0 pb-24 pt-0 space-y-4">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm space-y-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
            <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500">Foto oficial KroniX</div>
            <div className="mt-3 flex items-center gap-3">
              <OfficialDriverAvatar src={avatarSrc} initials={avatar} size={64} />

              <div className="min-w-0 flex-1">
                <div className="text-sm font-extrabold text-slate-900">Gestionada por KroniX</div>
                <div className="mt-1 text-[12px] text-slate-600">
                  Por seguridad, la foto del conductor no se cambia desde esta app. Si necesitas actualizarla, contacta a soporte o CTCC.
                </div>
              </div>
            </div>
          </div>
          ...
          <p className="text-[11px] text-gray-500">
            La foto visible para clientes es oficial y se carga desde <span className="font-semibold">public/branding/Driver_Pictures</span>.
          </p>
        </div>
      </div>
    </div>
